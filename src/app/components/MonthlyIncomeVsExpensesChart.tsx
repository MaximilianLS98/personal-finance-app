...
const MonthlyIncomeVsExpensesChart: React.FC = () => {
	...
	const monthly = useMemo(() => {
		...
		const monthMap = new Map<
			string,
			{ monthLabel: string; income: number; expenses: number }
		>();
		for (const item of src) {
			...
			const existing = monthMap.get(ymKey) ?? { monthLabel: label, income: 0, expenses: 0 };
			existing.income += Math.max(0, item.income || 0);
		...
		return sorted.map((k) => ({
			...
			income: monthMap.get(k)!.income,
			expenses: monthMap.get(k)!.expenses,
		}));
	}, [data]);
